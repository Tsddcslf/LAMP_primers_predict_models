...
# 定义带有位置权重的损失函数类
class WeightedPositionCrossEntropyLoss(nn.Module):
    def __init__(self, weight=None, correct_weight=0.5, incorrect_weight=2.0):
        super(WeightedPositionCrossEntropyLoss, self).__init__()
        self.weight = weight  # 类别权重
        self.correct_weight = correct_weight  # 正确预测的权重系数
        self.incorrect_weight = incorrect_weight  # 错误预测的权重系数
        self.base_criterion = nn.CrossEntropyLoss(weight=weight, reduction='none')
    
    def forward(self, inputs, targets):
        # 输入形状: [batch_size*seq_len, num_classes]
        # 目标形状: [batch_size*seq_len]
        
        # 获取原始的交叉熵损失（不进行reduction）
        base_loss = self.base_criterion(inputs, targets)
        
        # 计算预测结果
        _, preds = torch.max(inputs, dim=1)
        
        # 创建权重掩码：根据预测是否正确应用不同的权重
        weight_mask = torch.ones_like(base_loss)
        weight_mask = torch.where(preds == targets, 
                                  self.correct_weight * weight_mask, 
                                  self.incorrect_weight * weight_mask)
...
dropout_rate = 0.3
epochs = 200
correct_weight = 0.1  # 正确预测的权重系数
incorrect_weight = 3.0  # 错误预测的权重系数
...
def train_model(model, train_loader, criterion, optimizer, device):
    ...
    correct_preds = 0
    total_preds = 0
    correct_pos = 0  # 正确预测的位置数
    incorrect_pos = 0  # 错误预测的位置数
    ...
    for batch in tqdm(train_loader, desc="Training"):
        ...
        # 统计正确和错误的位置数量
        correct_pos += torch.sum(correct_mask)
        incorrect_pos += torch.sum(~correct_mask)
    ...
    # 记录正确和错误预测的比例
    correct_ratio = (correct_pos / total_preds).item()
    incorrect_ratio = (incorrect_pos / total_preds).item()
    logger.info(f"训练中正确预测比例: {correct_ratio:.4f}, 错误预测比例: {incorrect_ratio:.4f}")
...
def validate_model(model, val_loader, criterion, device):
    ...
    correct_ratio = (correct_pos / total_preds).item()
    incorrect_ratio = (incorrect_pos / total_preds).item()
    logger.info(f"验证中正确预测比例: {correct_ratio:.4f}, 错误预测比例: {incorrect_ratio:.4f}")
